...
- Desktop result: **PASS**
- Physical-iPad result: **BLOCKED — no physical device connected**
...
| Physical iPad native handles and latency           | BLOCKED                            | A connected physical iPad is required                               |
...
5. Verify error announcements and the Snapshot fallback action.
6. Attach device identity, screen recording, and timing output to this Slice.

## Source Manifest

See `source-manifest.md`. Desktop evidence is local-only and contains no external service,
credential, or telemetry dependency.
